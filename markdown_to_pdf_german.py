#!/usr/bin/env python3
"""
Script to convert the README_german.md file to a well-formatted PDF with embedded images.
...
def create_image_flowable(image_path, max_width=6*inch, max_height=4*inch):
    ...
    if not os.path.exists(image_path):
        ...
        return Paragraph(f"[Bild nicht gefunden: {image_path}]", getSampleStyleSheet()['Normal'])
    ...
    try:
        ...
        print(f"📷 Füge Bild hinzu: {image_path} ({orig_width}x{orig_height} → {int(final_width)}x{int(final_height)})")
        
        return Image(image_path, width=final_width, height=final_height)
        
    except Exception as e:
        print(f"❌ Fehler beim Verarbeiten des Bildes {image_path}: {e}")
        return Paragraph(f"[Bildfehler: {image_path}]", getSampleStyleSheet()['Normal'])

def markdown_to_pdf(input_file='README_german.md', output_file='Spiele_Portfolio_German.pdf'):
    """Convert German markdown file to PDF with proper formatting"""
    ...
    print(f"📄 Verarbeite {len(md_content):,} Zeichen aus {input_file}")
    
    # Extract images before cleaning
    images = extract_images_from_markdown(md_content)
    print(f"🖼️  {len(images)} Bilder zum Einbetten gefunden")
    ...
    for match in img_pattern.finditer(cleaned_content):
        ...
        if pre_text:
            ...
            for line in lines:
                ...
                # Main title (first # heading) - look for German portfolio title
                if line.startswith('# ') and ('Spiele-Portfolio' in line or 'Appendix' in line):
                    # German title
                    story.append(Paragraph("Anhang: Spiele-Portfolio 2005 bis 2024", title_style))
        ...
        print(f"🖼️  Verarbeite Bild: {image_path}")
    ...
    # Build PDF
    print(f"🔄 {image_count} Bilder während der Dokumentenerstellung verarbeitet")
    print(f"🔄 Erstelle PDF mit {len(story)} Elementen...")
    doc.build(story)
    
    # Get file size
    pdf_size = os.path.getsize(output_file)
    print(f"✅ PDF erfolgreich erstellt: {output_file}")
    print(f"📊 Dateigröße: {pdf_size:,} bytes ({pdf_size/1024:.1f} KB)")
    
    return True

def main():
    print("🎮 Deutsche Spiele-Portfolio Markdown zu PDF Konverter")
    print("=" * 55)
    ...
    # Convert German markdown to PDF
    success = markdown_to_pdf('README_german.md', 'Spiele_Portfolio_German_2024-2005.pdf')
    
    if success:
        print("\n🎉 Konvertierung erfolgreich abgeschlossen!")
        print("📁 Sie können jetzt Spiele_Portfolio_German_2024-2005.pdf teilen")
    else:
